...
public final class D4C {
    ...
    static void SetParametersForGetWindowedWaveform(int half_window_length,
        ...
        int origin = (int) Math.round(current_position * fs + 0.001);
    ...
    static void GetWindowedWaveform(final double[] x, int x_length, int fs,
        ...
        for (int i = 0; i <= half_window_length * 2; ++i)
        waveform[i] =
        x[safe_index[i]] * window[i] + random.randn() * ConstantNumbers.kMySafeGuardMinimum;
    ...
    static void GetCentroid(final double[] x, int x_length, int fs,
        ...
        current_position, ConstantNumbers.kBlackman, 4.0, forward_real_fft.waveform, random);
        double power = 0.0;
    ...
    static void GetSmoothedPowerSpectrum(final double[] x, int x_length, int fs,
        double current_f0, int fft_size, double current_position,
        final Common.ForwardRealFFT forward_real_fft, double[] smoothed_power_spectrum
        , MatlabFunctions.Random random
    ) {
        for (int i = 0; i < fft_size; ++i) forward_real_fft.waveform[i] = 0.0;
        GetWindowedWaveform(x, x_length, fs, current_f0,
        current_position, ConstantNumbers.kHanning, 4.0, forward_real_fft.waveform, random);
    ...
    static void GetStaticGroupDelay(final double[] static_centroid,
        ...
        Common.LinearSmoothing(static_group_delay, f0 / 2.0, fs, fft_size,
        static_group_delay);

        double[] smoothed_group_delay = new double[fft_size / 2 + 1];
        Common.LinearSmoothing(static_group_delay, f0, fs, fft_size,
    ...
    //-----------------------------------------------------------------------------
    static void GetCoarseAperiodicity(final double[] static_group_delay, int fs,
        ...
        int boundary =
        (int) Math.round(fft_size * 8.0 / window_length);
        ...
        center =
            (int) (ConstantNumbers.kFrequencyInterval * (i + 1) * fft_size / fs);
    ...
    ConstantNumbers.kBlackman, 3.0, forward_real_fft.waveform, random);
    ...
    static void D4CLoveTrain(final double[] x, int fs, int x_length,
        ...
        int fft_size = (int) (Math.pow(2.0, 1.0 +
        (int) (Math.log(3.0 * fs / lowest_f0 + 1) / ConstantNumbers.kLog2)));
    ...
    private static void D4CMain(final double[] x, int x_length, int fs,
        ...
        int fft_size_d4c = (int) (Math.pow(2.0, 1.0 +
        (int) (Math.log(4.0 * fs / ConstantNumbers.kFloorF0D4C + 1) /
